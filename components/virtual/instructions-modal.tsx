  ...
  return (
    <div className="fixed inset-0 bg-black/60 dark:bg-black/80 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-xl p-6 max-w-lg w-full shadow-xl relative">
        ...
        <button
          onClick={onClose}
          className="absolute top-3 right-3 text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 transition-all"
          aria-label="Close"
        >
          <X className="w-6 h-6" />
        </button>
        ...
        <div className="text-center mb-6">
          <h3 className="text-2xl font-semibold text-gray-800 dark:text-gray-100">How Virtual Try-On Works</h3>
        </div>
        ...
        <div className="space-y-6">
          {[
            ...
            },
            {
              ...
              icon: <Shirt className="w-6 h-6 text-green-600 dark:text-green-400" />,
            },
            {
              step: 4,
              title: 'Customize & Preview',
              description: 'Adjust the fit and color of garments. See how they look on you in real-time.',
              icon: <Eye className="w-6 h-6 text-red-600 dark:text-red-400" />,
            },
          ].map(({ step, title, description, icon }) => (
            <div key={step} className="flex items-start space-x-4">
              {/* Step Number with Gradient */}
              <div className="w-10 h-10 rounded-full bg-gradient-to-r from-purple-500 to-blue-500 dark:from-purple-400 dark:to-blue-400 flex items-center justify-center text-white font-semibold shadow-lg">
                {step}
              </div>
              {/* Step Content */}
              <div>
                <h4 className="font-medium text-gray-800 dark:text-gray-200 flex items-center space-x-2">
